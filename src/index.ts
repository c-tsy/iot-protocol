...
export class V0 extends Base {
    ...
    encode(explain: boolean = false): Buffer {
        ...
        //左移6位
        buf[3].writeUInt16BE(this.Control << 6, 0);
    ...
    decode() {
        super.decode();
        //读出厂商代码
        this.explain.push(explain_text(this.buf, 0, 1, '起始位置', 'Start', '0x68'));
        this.explain.push(explain_text(this.buf, 1, 1, '版本号', 'Version', this.Version));
        let i = 2;
        this._CompanyID = this.buf.slice(i, i += 3).toString('hex');
        this.explain.push(explain_text(this.buf, i - 3, 3, '厂商代码', 'CompanyID', this._CompanyID));
        this.Control = this.buf.readUInt16BE(i) >> 6;
        this.explain.push(explain_text(this.buf, i, 2, '控制码(2位,编码左移,解码右移) >> 6', 'Control', this.Control));
        let t = i + 4;
        i++;
        if (this.buf[i] & 1) {
            this.Next = true;
        ...
        this.explain.push(explain_text(this.buf, i, 1, '是否加密[第3位],1加密0不加密', 'Encrypted', this.Encrypted));
        if (this.buf[i] & 8) {
            this.Confirm = true;
        }
        this.explain.push(explain_text(this.buf, i, 1, '是否加密[第4位],1需要确认0不需确认', 'Confirm', this.Confirm));
        this.explain.push(explain_text(this.buf, i, 1, '接收方地址类型[第5位],1物理0逻辑', 'To.Type', '4字节为物理地址2字节为逻辑地址'));
        this.explain.push(explain_text(this.buf, i, 1, '接收方地址类型[第6位],1物理0逻辑', 'From.Type', '4字节为物理地址2字节为逻辑地址'));
        if (this.buf[i] & 32) {
            this.From.Type = AddressType.Phy;
            this.From.Value = Buffer.from(this.buf.slice(t, t += 4)).reverse().toString('hex');
            this.explain.push(explain_text(this.buf, t - 4, 4, '发送方地址', 'From.Value', this.From.Value));
        } else {
            this.From.Type = AddressType.Logic;
            this.From.Value = Buffer.from(this.buf.slice(t, t += 2)).reverse().toString('hex');
            this.explain.push(explain_text(this.buf, t - 2, 2, '发送方地址', 'From.Value', this.From.Value));
        }
        if (this.buf[i] & 16) {
            this.To.Type = AddressType.Phy;
            this.To.Value = Buffer.from(this.buf.slice(t, t += 4)).reverse().toString('hex');
            this.explain.push(explain_text(this.buf, t - 4, 4, '接收方地址', 'To.Value', this.To.Value));
        } else {
            this.To.Type = AddressType.Logic;
            this.To.Value = Buffer.from(this.buf.slice(t, t += 2)).reverse().toString('hex');
            this.explain.push(explain_text(this.buf, t - 2, 2, '接收方地址', 'To.Value', this.To.Value));
        }
        i++;
        //帧序号和帧内顺序
        this.ID = this.buf.readUInt16LE(i++) >> 7;
        this.explain.push(explain_text(this.buf, i - 1, 2, '帧序号 解码>>7,编码<<7', 'ID', this.ID));
        if (this.buf[i] & 64) {
            this.End = false;
        }
        this.explain.push(explain_text(this.buf, i, 1, '是否结束[第7位],1未结束0结束', 'End', this.End));
        this.No = this.buf[i++] & 63;
        this.explain.push(explain_text(this.buf, i - 1, 1, '包内第几帧[低6位]', 'No', this.No));
        i = t;
        /**
         * 数据区协议
         */
        this.DataType = this.buf[i++];
        this.explain.push(explain_text(this.buf, i - 1, 1, '数据区协议类型', 'DataType', this.DataType));
        let len = this.buf.readUInt16LE(i);
        this.explain.push(explain_text(this.buf, i, 2, '数据区长度', 'DataLen', len));
        i += 2;
        ...
        this.explain.push(explain_text(this.buf, i, 2, '数据区内容', 'Data', buffer2hex(this.Data)));
        return this;
    }
}
/**
